...
1. **Neurohub на том же VPS, что и c-fab** — обращение к **публичному** `https://ai.nova01.click` с самого сервера иногда ломается (**hairpin NAT**: наружный IP с внутренней стороны не маршрутизируется). Решение: в **`/opt/c-fab-app/.env`** (и при необходимости в LLM-профиле проекта) указать **внутренний** URL, например `http://127.0.0.1:<порт>/neurohub/v1` или имя сервиса в Docker-сети, **не** публичный домен.
2. **Neurohub на другой машине** — в файрволе / security group должен быть разрешён **исходящий** с nova-02 **или входящий** на хост Neurohub с IP nova-02 (смотря где фильтр).
3. **Сервис не слушает 443** или слушает только `localhost` — снаружи и с c-fab будет таймаут.
