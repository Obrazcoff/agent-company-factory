...
## LLM / Neurohub: `ConnectTimeoutError` / `fetch failed`

Сообщение вида **`LLM fetch failed for https://ai....../chat/completions`** с **`Connect Timeout`** значит: процесс **c-fab на VPS** не смог открыть **TCP:443** до хоста из **`NEUROHUB_BASE_URL`** (или из профиля проекта в БД). До HTTP-запроса дело не доходит — это не «не тот JSON» и не ключ API.

**Проверка с того же сервера, где крутится Next:**

```bash
ssh nova-02 'curl -sv --connect-timeout 5 -o /dev/null https://ai.nova01.click/ 2>&1 | head -40'
```

Если `curl` тоже висит или таймаут — чинить **сеть / файрвол / DNS / куда слушает Neurohub**, а не код c-fab.

**Соответствие доке Neurohub (onboarding клиента):**

- Клиентский **Base URL** в их доке: `https://ai.nova01.click/neurohub/v1` → в c-fab это **`NEUROHUB_BASE_URL`** (или поле base URL в `/settings/llm` для проекта).
- **`NEUROHUB_API_KEY`** — выданный **`api_key`** пользователя (не admin-ключ из `ADMIN_API_KEYS`).
- Альтернатива в их доке для **mag-01**: `http://127.0.0.1:6006` — это шлюз **только на localhost mag-01**. Для приложения на **nova-02** такой URL не подходит; нужен либо публичный `https://ai.nova01.click/...` с доступом **с IP nova-02**, либо приватный адрес mag-01 в вашей сети (Docker bridge, WireGuard, tailnet и т.д.), **не** `127.0.0.1` на nova-02.

**Частые случаи:**

1. **c-fab на nova-02, Neurohub на mag-01 / за другим IP** — хост `ai.nova01.click` должен принимать **TCP с egress-IP nova-02** (ufw/security group на стороне шлюза). Твой `curl` с nova-02 до `78.29.49.129:443` с таймаутом = этот кейс, пока не откроешь путь или не дашь внутренний URL.
2. **Neurohub на том же VPS, что и c-fab** — обращение к **публичному** `https://ai.nova01.click` с самого сервера иногда ломается (**hairpin NAT**: наружный IP с внутренней стороны не маршрутизируется). Решение: в **`/opt/c-fab-app/.env`** (и при необходимости в LLM-профиле проекта) указать **внутренний** URL, например `http://127.0.0.1:<порт>/neurohub/v1` или имя сервиса в Docker-сети, **не** публичный домен.
3. **Сервис не слушает 443** или слушает только `localhost` на публичном IP — снаружи и с c-fab будет таймаут.

После правок **`systemctl restart c-fab-app`**.
